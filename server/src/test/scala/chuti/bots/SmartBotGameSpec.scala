...
object SmartBotGameSpec extends ZIOSpecDefault {

  // Mock LLM service that uses DumbBot logic to ensure valid moves
  class DumbBotBasedLLMService extends LLMService[GameError] {
  ...
  def spec = suite("SmartBotGameSpec")(
    test("SmartBot can play through cantando phase without errors") {
      ...
      for {
        ...
        cantaEvent <- smartBot.decideTurn(turnoPlayer.user, game)

      } yield assertTrue(
        cantaEvent.isInstanceOf[Canta]
      )
    },
    test("SmartBot handles requiereSopa phase gracefully") {
      val config = OllamaConfig(
        baseUrl = "http://localhost:11434",
        modelName = "test",
        temperature = 0.7,
      ...
      for {
        sopaEvent <- smartBot.decideTurn(turnoPlayer.user, game)
      } yield assertTrue(
        sopaEvent.isInstanceOf[Sopa]
      )
    },
    test("SmartBot makes valid moves that don't violate game rules") {
      val config = OllamaConfig(
        baseUrl = "http://localhost:11434",
        modelName = "test",
        temperature = 0.7,
