...
import chuti.*
import chuti.api.token.{TokenHolder, TokenPurpose}
import chuti.bots.{AIBot, ClaudeBot, LLMStats}
import chuti.chat.ChatService
import db.*
import db.quill.QuillRepository
...
object EnvironmentBuilder {
  ...
  private val aiBotLayer: ZLayer[ai.LLMService & ConfigurationService, ConfigurationError, Option[ClaudeBot]] = ZLayer
    .fromZIO {
      for {
      ...
      } yield config.map(c => ClaudeBot()) // AIBot(config = c.ollama, llmService = llmService, stats = statsRef))
    }
