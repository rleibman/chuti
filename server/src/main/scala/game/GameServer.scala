...
trait GameServer extends Repository {
  val god: User = User(id = Some(UserId(-666)), email = "god@chuti.fun", name = "Un-namable", wallet = Double.MaxValue)
  private val zioEventQueue: UIO[Queue[(GameState, Jugador, Event)]] = ZQueue.unbounded[(GameState, Jugador, Event)]
  ...
  def event(game: GameState, jugador: Jugador, event: Event): RepositoryIO[GameState] = ???
  ...
  def changePassword(user: User, password: String): RepositoryIO[Boolean] = ???
  def login(user: User, password: String): RepositoryIO[Boolean]          = ???
  def inviteFriend(user: User, friend: User): RepositoryIO[Boolean]       =
    ...
    //  Add a temporary record in the friends table
    ???
  def acceptFriendship(user: User, friend: User): RepositoryIO[Boolean] =
    ???
