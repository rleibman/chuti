...
trait ChatRoute extends Directives with HasActorSystem {
  ...
  def route: RIO[Console & Clock & ChatService & Repository & SessionContext & Logging & Config, Route] = {
    for {
      config <- ZIO.service[Config.Service]
      runtime <-
        ZIO
          .runtime[
            Console & Clock & ChatService & Repository & SessionContext & Logging & Config
          ]
    } yield {
      val staticContentDir =
        config.config.getString(s"${config.configKey}.staticContentDir")

      implicit val r: zio.Runtime[Console & Clock & ChatService & Repository & SessionContext & Logging & Config] = runtime
